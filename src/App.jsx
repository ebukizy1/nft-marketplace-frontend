                    ...
                    AllCollections={
                        <Flex align="center" gap="8" wrap={"wrap"}>
                            {tokensData.length === 0 ? (
                                ...
                                tokensData.map((x) => {
                                    const isOwned = myTokenIds.includes(x.index); // Check if the token is owned
                                    // const myTokenData = myTokensData.find((y) => y.dna === x.dna); // Find the token data if owned
                                    ...
                                    return (
                                        <Box key={x.dna} className="w-[20rem]">
                                            ...
                                            {isOwned ? ( // Render Transfer button if owned, Mint button if not
                                                <Button className="px-8 py-2 text-xl mt-2">
                                                    Transfer
                                                </Button>
                                            ) : (
